...
final class coreDataManager {
    ...
    func getToDoListFromCoreData() -> [ToDoData] {
        ...
        if let context = context {
            // 요청서
            let request = NSFetchRequest<NSManagedObject>(entityName: CoreData.modelName)
            // 정렬순서를 정해서 요청서에 넘겨주기
    ...
    func saveToDoData(toDoText: String?, colorInt: Int64, completion: @escaping () -> Void) {
        ...
        if let context = context {
            ...
            if let entity = NSEntityDescription.entity(forEntityName: CoreData.modelName, in: context) {
